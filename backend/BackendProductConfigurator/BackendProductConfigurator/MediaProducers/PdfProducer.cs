    ...
    public static class PdfProducer
    {
        ...
        public static void GeneratePDF(ConfiguredProduct product, string configId, HttpRequest request)
        {
            try
            {
                ...
                Configurator configurator = ValuesClass.Configurators[GlobalValues.Languages.First()].Where(con => con.ConfigId == configId).First();
                Option tempOption;
                ...
                foreach (string optionId in product.Options)
                {
                    ...
                    PrintOption(tf, font, page, page.Width * (leftBorder + 0.04), page.Width * 0.66, yPosition, $"- {tempOption.Name} ({tempOption.ProductNumber})", price);
                    yPosition += smallSpacing;
